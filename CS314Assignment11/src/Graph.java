...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        if (!containsVertex(startName)) {
            throw new NoSuchElementException("No Vertex named " + startName + " exists in Graph");
        }
        currentStartVertexName = startName;
        // get the start vertex from the map of vertices
        Vertex startVtx = vertices.get(startName);
        clearAll();
        // set vertex instance variables to 0
        startVtx.numEdgesFromStartVertex = 0;
        startVtx.weightedCostFromStartVertex = 0;
        // create a priority queue and add path from start vertex
        PriorityQueue<Path> pq = new PriorityQueue<>();
        pq.add(new Path(startVtx, 0));
        while (!pq.isEmpty()) {
        	// dequeue first element from priority queue
        	Path p = pq.remove();
        	Vertex v = p.dest;
        	if (v.scratch == 0) {
        		// increment to show vertex has been visited
        		v.scratch++;
        		for (Edge e : v.adjacent) {
        			Vertex temp = e.dest;
        			double sum = e.cost + v.weightedCostFromStartVertex;
        			if (temp.weightedCostFromStartVertex >= sum) {
        				temp.prev = v;
        				temp.weightedCostFromStartVertex = sum;
        				// increase the number of edges
        				temp.numEdgesFromStartVertex = v.numEdgesFromStartVertex + 1;
        				// add to priority queue with weighted cost
        				pq.add(new Path(temp, temp.weightedCostFromStartVertex));
        			} 
        		}
        	}
        }
    ...
    public void findAllPaths(boolean weighted) {
    	...
    	for (String vName : vertices.keySet()) {
    		...
    		// reset path info
    		v.clearPathInfo();
    		// call dijkstra if weighted, else call findUnweighedShortestPath
    		if (weighted) {
    			dijkstra(vName);
    		} else {
    			findUnweightedShortestPath(vName);
    		}
    		for (String val : vertices.keySet()) {
    			// checks that the vertices are connected
				if (getNumEdgesFromStart(val) > 0) {
					v.numVertexConnected++;
					v.totalWeightedPathLength += getWeightedCostFromStart(val);
					v.totalUnweightedPathLength += getNumEdgesFromStart(val);
					// if weighted cost with val is greater than longest, update longest
					if (getWeightedCostFromStart(val) > longest.weightedCost()) {
						longest = getPath(val);
					}
				}
			}
