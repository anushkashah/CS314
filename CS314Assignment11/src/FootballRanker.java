...
public class FootballRanker {
    ...
    private final static double ROUND = 10.0;
    ...
    public double doWeightedAndWinPercentAdjusted(boolean showResults) {
        if (showResults) {
            System.out.print("\n\n ***** RESULTS BASED ON WEIGHTED WINS ADJUSTED BY WIN"
            		+ " PERCENTAGE *****");
        }
        teamsAndResults.findAllPaths(true);
        TreeSet<AllPathsInfo> paths = teamsAndResults.getAllPaths();
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        double result = 0;
        // iterate through array list human ranks
        for (int i = 0; i < humanRanks.size(); i++) {
        	String actual = humanRanks.get(i);
        	// get predicted rank
        	int predRank = getPredicted(actual, paths);
        	// 1 based indexing so need to increase i by 1
        	int apRank = i + 1;
        	// subtract predRank from apRank and square it
        	result += Math.pow(apRank - predRank, 2);
        	if (showResults) {
        		System.out.printf("%-21s- actual rank: %d predicted rank: %d\n",
        				actual, apRank, predRank);
        	}
        }
    ...
    /**
     * Get the predicted rank
     * @param actual name of team
     * @param paths ranks from graph stored treeset
     * @return predicted rank of team
     */
    private int getPredicted(String actual, TreeSet<AllPathsInfo> paths) {
    	int val = 1;
    	// iteratate through each element in paths
    	for (AllPathsInfo path : paths) {
    		// check if path name is the same as team name
    		if (path.getName().equals(actual)) {
    			return val;
    		}
    		val++;
    	}
